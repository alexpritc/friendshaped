...
namespace Player
{
    public class Interactions : MonoBehaviour
    {
        ...
        private void OnTriggerEnter2D(Collider2D collision)
        {
            ...
            {
                GameManager.Instance.CreatePrompt(transform.position + new Vector3(2, 2, 0), "Use Door", PromptKeys.E);
            }
            else if (collision.tag == "NPC")
            {
                GameManager.Instance.CreatePrompt(transform.position + new Vector3(2, 2, 0), "Talk", PromptKeys.E);
            }
            else if (collision.tag == "Item")
            {
                GameManager.Instance.CreatePrompt(transform.position + new Vector3(2, 2, 0), "Inspect", PromptKeys.E);
            }
        }
